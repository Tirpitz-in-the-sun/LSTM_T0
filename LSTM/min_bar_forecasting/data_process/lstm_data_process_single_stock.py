...
class SampleConstructor(object):
    ...
    def __construct_standard_data(self, time_step, usage='train', sday=0):
        ...
        for t in range(data_set['intra_close'].shape[0]):
            ...
            for z in range(237-time_step):
                ...
                standard_data_output.append([output_data])
                ...
                self.standard_data_output_dict[trading_date][z + time_step] = (output_data*self.price_std +
                                                                               self.price_mean)*pre_close_date
                volatility = (abs(output_data-pre_data))/(pre_data+self.price_mean/self.price_std)
        ...
        vola_standard = 0.002  # 打败交易费
        self.large_vol_index = np.where(self.volatility_output > vola_standard)[0]
        self.small_vol_index = np.where(self.volatility_output < vola_standard)[0]
    ...
    def construct_batch_data(self, time_step, batch_size, large_volatility_prop=0.5):
        ...
        index_large = random.sample(self.large_vol_index, large_batch_size)
        index_small = random.sample(self.small_vol_index, small_batch_size)
        index = index_large + index_small
...
if __name__ == '__main__':
    ...
    all_value = sample_constructor.train_init_data['intra_value']
    all_volume = sample_constructor.train_init_data['intra_volume']
    pd.DataFrame(all_value).to_csv("intra_value_%d.csv" % symbol)
    pd.DataFrame(all_volume).to_csv("intra_volume_%d.csv" % symbol)
    # # sample_constructor.construct_batch_data(15, 100, 0.5)
