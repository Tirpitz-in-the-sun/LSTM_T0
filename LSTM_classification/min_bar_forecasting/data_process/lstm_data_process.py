...
class MultiCodeSampleConsructor(object):

    def __init__(self, code_array, train_end_date):

        self.sample_constructor_array = []

        for code in code_array:
            self.sample_constructor_array.append(SampleConstructor(code, train_end_date))

    def construct_batch_data(self, time_step, batch_size, large_volatility_prop=0.5):

        x_result = []
        y_result = []
        batch_size_each = batch_size/len(self.sample_constructor_array)

        for sample_constructor in self.sample_constructor_array:
            train_each = sample_constructor.construct_batch_data(time_step, batch_size_each, large_volatility_prop)
            x_result.extend(train_each[0])
            y_result.extend(train_each[1])
...
if __name__ == '__main__':
    ...
    sample_constructor = MultiCodeSampleConsructor(symbol, 20161231)
